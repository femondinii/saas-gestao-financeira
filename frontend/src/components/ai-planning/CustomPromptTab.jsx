...
import {
    ...
    sanitizePromptLocal
} from "../../utils/financialPromptValidator";
...
export default function CustomPromptTab({ onSaved }) {
    ...
    const canGenerate = useMemo(
        () => customPrompt.trim().length > 0 && issues.length === 0,
        [customPrompt, issues]
    );
    ...
    const handleGenerateFromPrompt = async () => {
        ...
        try {
            ...
            const res = await api.post("/finance/ai/plan/generate/", body, {
                withAuth: true
            });
            ...
            const plan = data.title
                ? data
                : { title: "Plano Personalizado", spec: data.spec || {} };
        ...
        <div className="space-y-4 relative pb-4">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Descreva em detalhes o que você gostaria de planejar
                        financeiramente e nossa IA criará um plano personalizado para você.
                    </CardDescription>
                </CardHeader>
                    ...
                    <div className="space-y-2">
                        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                            <label className="text-sm font-medium">
                                Descreva seu objetivo financeiro
                            </label>
                            <span
                                className={`text-xs ${
                                    chars > 2000
                        ...
                        <textarea
                            ...
                            className="min-h-[150px] w-full rounded-lg border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                            value={customPrompt}
                        ...
                        <p className="text-xs text-muted-foreground">
                            Seja específico: inclua valores, prazos, sua situação atual e
                ...
                <CardFooter>
                    <Button
                        ...
                        className="w-full sm:w-auto inline-flex items-center gap-2 px-4 py-3"
                    >
                        {loading ? (
                            "Processando…"
                        ) : (
